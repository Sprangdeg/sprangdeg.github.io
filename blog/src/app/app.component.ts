...
import { Subject, filter, from, interval, map, of, switchMap, takeUntil, tap } from 'rxjs';
...
export class AppComponent implements OnInit {
  ...
  onApplyFilter() {
    this.filterNumbers$
      .pipe(
        tap(numbers => this.filterOutputNumbers = numbers.filter(number => number%2 === 0))
      )
      .subscribe();
  }
  ...
  onApplyFrom() {
    ...
    this.outputDataFrom = [this.fromQueue.shift() || ''];
  }
  ...
  ngOnInit() {
    this.switchMapData$.pipe(
      switchMap(() => interval(100).pipe(takeUntil(this.stopTimer$)))
    ).subscribe(
